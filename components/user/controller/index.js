...
//     let friendOfFollowers = await UserModel.find(
//       { _id: arraysId },
//       { select: follows }
//     ).populate({
//       path: "follows",
...
//     let filter1 = [];
//     toFollows.map((valor) => {
//       let esta = arraysId.find(
//         (v) => JSON.stringify(v) === JSON.stringify(valor._id)
//       );
//       if (!esta) {
//         filter1.push(valor);
//       }
...
//       filtradoToFollows = await UserModel.find({ _id: { $ne: user } }).limit(
//         15
//       );
